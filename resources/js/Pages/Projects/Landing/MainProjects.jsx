...
export default function MainProjects({ projects, highlight }) {
    ...
    const handleSearchChange = async (project) => {
        const value = project.target.value;
        await setData('search', value);
        router.visit(route('MainProjects'),
            {
                method: 'get',
                data: {
                    search: value
    ...
    return (
        <GuestLayout>
            <div className="bg-white font-poppins">
                ...
                {/* Banner Start */}
                <BannerProjects images={highlight} />
                {/* Banner End */}
                <Jarak tinggi={'h-10'} />
                        ...
                        <div className="md:grid md:grid-cols-2 md:gap-4 lg:grid-cols-none max-w-screen-xl mx-auto px-6 lg:px-8 relative">
                            {projects.data.map((data) => (
                                <MainCard key={data.id}>
                                    <MainCard.Header image={`/storage/${data.image}`} alt={data.alt} />
                                    <MainCard.Body
                                        title={data.title}
                                        slug={data.slug}
                                    ...
                                    >{data.description}</MainCard.Body>
                                </MainCard>
                            ))}
                        </div>
                        {/* Card End */}
